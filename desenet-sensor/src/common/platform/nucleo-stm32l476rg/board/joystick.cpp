...
Joystick::Joystick(const Joystick &) : XFReactive() {
    _bInitialized = false;
    _pObserver = NULL;
    _position.position = Invalid;
    _currentState = STATE_INITIAL;
...
void Joystick::start() { startBehavior(); }
...
IJoystick::Position Joystick::position() const { return _position; }
...
Joystick::PressedButtons Joystick::_readActualPosition() const {
    ...
    pressed |=
        (HAL_GPIO_ReadPin(NAVS_UP_GPIO_Port, NAVS_UP_Pin) == GPIO_PIN_RESET)
            ? JoystickButton::Up
            : JoystickButton::Invalid;

    // Check DOWN button pressed
    pressed |=
        (HAL_GPIO_ReadPin(NAVS_DOWN_GPIO_Port, NAVS_DOWN_Pin) == GPIO_PIN_RESET)
            ? JoystickButton::Down
            : JoystickButton::Invalid;

    // And also for the remaining buttons
    pressed |=
        (HAL_GPIO_ReadPin(NAVS_LEFT_GPIO_Port, NAVS_LEFT_Pin) == GPIO_PIN_RESET)
            ? JoystickButton::Left
            : JoystickButton::Invalid;
    pressed |= (HAL_GPIO_ReadPin(NAVS_RIGHT_GPIO_Port, NAVS_RIGHT_Pin) ==
                GPIO_PIN_RESET)
                   ? JoystickButton::Right
                   : JoystickButton::Invalid;
    pressed |= (HAL_GPIO_ReadPin(NAVS_CENTER_GPIO_Port, NAVS_CENTER_Pin) ==
                GPIO_PIN_RESET)
    ...
    switch (_currentState) {
        ...
        case STATE_CHECK_JOYSTICK: {
            ...
            if (getCurrentEvent()->getEventType() == IXFEvent::Event &&
                ...
                if (!_timeoutActive) {
                    getThread()->scheduleTimeout(Timeout_CHECK_JOYSTICK_id, 50,
                                                 this);
                }
...
void Joystick::doCheckJoystickState() {
    ...
    if (oldPosition.pressedButtons != _position.pressedButtons) {
        ...
        if ((oldPosition.pressedButtons & JoystickButton::Up) !=
            (_position.pressedButtons & JoystickButton::Up)) {
            traceOut("Joystick button up %s",
                     (_position.pressedButtons == JoystickButton::Up)
                         ? "pressed"
                         : "released");
        }

        if ((oldPosition.pressedButtons & JoystickButton::Down) !=
            (_position.pressedButtons & JoystickButton::Down)) {
            traceOut("Joystick button down %s",
                     (_position.pressedButtons == JoystickButton::Down)
                         ? "pressed"
                         : "released");
        }

        if ((oldPosition.pressedButtons & JoystickButton::Left) !=
            (_position.pressedButtons & JoystickButton::Left)) {
            traceOut("Joystick button left %s",
                     (_position.pressedButtons == JoystickButton::Left)
                         ? "pressed"
                         : "released");
        }

        if ((oldPosition.pressedButtons & JoystickButton::Right) !=
            (_position.pressedButtons & JoystickButton::Right)) {
            traceOut("Joystick button right %s",
                     (_position.pressedButtons == JoystickButton::Right)
                         ? "pressed"
                         : "released");
        }

        if ((oldPosition.pressedButtons & JoystickButton::Pressed) !=
            (_position.pressedButtons & JoystickButton::Pressed)) {
            traceOut("Joystick button center %s",
                     (_position.pressedButtons == JoystickButton::Pressed)
                         ? "pressed"
                         : "released");
        }
