...
desenet::Frame::Frame(size_t initialLength)
    : buffer_(initialLength), frameLength_(0) {
    assert(buffer_.data());
...
desenet::Frame desenet::Frame::useBuffer(std::uint8_t *buffer,
                                         std::size_t length) {
    return Frame(SharedByteBuffer::proxy(buffer, length));
}

const desenet::Frame desenet::Frame::useBuffer(const uint8_t *const buffer,
                                               size_t length) {
    return Frame(SharedByteBuffer::proxy(buffer, length));
}

desenet::Frame desenet::Frame::copyFromBuffer(const uint8_t *const buffer,
                                              size_t length) {
...
bool desenet::Frame::isValid() const {
    return !buffer_.empty() &&
           frameLength_ >=
               Frame::HEADER_SIZE + 1 &&  // + 1 for byte containing frame type
...
void desenet::Frame::setPayloadSize(uint8_t payloadSize) {
    memcpy(buffer_.data() + desenet::Address::addressSize(), &payloadSize,
           sizeof(payloadSize));
}
...
desenet::FrameType desenet::Frame::type() const {
    ...
    else if (frameLength_ >= 1 &&
             ((buffer_[Frame::HEADER_SIZE + 0] & 0x80) == 0x80))
...
const char *desenet::Frame::toString() const {
    return toString(buffer_.data(),
                    length());  // Call the static version of toString()
}

// static
const char *desenet::Frame::toString(const uint8_t *const buffer,
